...
#define ADC_VAL_ZERO_CLI(ADC_VAL_LIMIT)                                                                                \
	do                                                                                                                 \
	{                                                                                                                  \
...
void adc_sample_thread_entry(void *parm)
{
	...
	while (1)
	{
		if ((RELAY_STATUS_OFF == g_ccr_data.ccr_info.ccr_relay_state))
		{
			// update_i_load(0);
			// update_load_voltage(0);

			// g_ccr_data.ccr_info.local_active_power = 0;
		}
		else
		{
			g_ccr_data.ccr_info.local_active_power = (uint32_t)active_power;
		}
